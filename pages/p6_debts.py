        ...
        with c_inv_add:
            ...
            if debts_for_inv:
                ...
                if st.button("Lưu Hóa Đơn", type="primary", use_container_width=True):
                    if so_hoa_don:
                        st.session_state["confirm_save_inv"] = {
                            "d_inv": d_inv,
                ...
                if "confirm_save_inv" in st.session_state:
                    data = st.session_state["confirm_save_inv"]
                    if data["d_inv"]["ma_hd"] == d_inv["ma_hd"]:
                        st.markdown(f"""
                        <div style="background:#fffbeb;border:1px solid #fcd34d;padding:12px;border-radius:8px;margin-top:10px;">
                            <b style="color:#b45309;">⚠️ Xác nhận:</b><br>
                            Bạn có chắc chắn muốn lưu hóa đơn <b>{data['so_hoa_don']}</b> cho <b>{data['d_inv']['ten_cty']}</b>?
                        </div>
                        """, unsafe_allow_html=True)
                        
                        c1, c2 = st.columns(2)
                        if c1.button("✅ Đồng ý", use_container_width=True, key="btn_yes_inv"):
                            gia_truoc_vat = data['d_inv']['can_thu'] - data['d_inv']['tien_vat']
                            vat_pct = (data['d_inv']['tien_vat'] / gia_truoc_vat * 100) if gia_truoc_vat > 0 else 0
                            conn.execute("""
                                INSERT INTO invoices (ma_hd, ma_kh, ky_thang, so_hoa_don, ngay_xuat, gia_truoc_vat, vat_pct, tien_vat, tong_tien, trang_thai)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                            """, (data['d_inv']['ma_hd'], data['d_inv']['ma_kh'], data['d_inv']['ky_thanh_toan'], data['so_hoa_don'], data['ngay_xuat'].isoformat(), gia_truoc_vat, vat_pct, data['d_inv']['tien_vat'], data['d_inv']['can_thu'], data['trang_thai']))
                            conn.commit()
                            del st.session_state["confirm_save_inv"]
                            st.success("Tạo hóa đơn thành công!"); st.rerun()
                        
                        if c2.button("❌ Hủy", use_container_width=True, key="btn_no_inv"):
                            del st.session_state["confirm_save_inv"]
                            st.rerun()
            else:
