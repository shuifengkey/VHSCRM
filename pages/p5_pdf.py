    ...
    if customer and contract:
                ...
                try:
                    ...
                    with st.spinner("⏳ Đang sinh PDF..."):
                        if "Báo Giá" in doc_type:
                            ...
                            filename = f"BaoGia_{selected_ma_kh}_{datetime.now().strftime('%Y%m%d')}.pdf"
                        elif "Hợp Đồng" in doc_type:
                            pdf_bytes = generate_hop_dong(customer_dict, contract_dict)
                            filename = f"HopDong_{contract_dict['ma_hd']}_{datetime.now().strftime('%Y%m%d')}.pdf"
                        else:
                            # Phiếu xác nhận
                            ...
                            filename = f"PhieuXacNhan_{selected_ma_kh}_{datetime.now().strftime('%Y%m%d%H%M')}.pdf"
        ...
        with col_info:
            ...
            <div style="padding:12px;background:#fffbeb;border-radius:8px;font-size:13px;">
                ...
                🕐 <b>Thời điểm xuất:</b> {datetime.now().strftime('%H:%M %d/%m/%Y')}<br>
