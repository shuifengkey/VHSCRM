...
def render():
    ...
    with tab_work:
        ...
        past3_str = (date.today()-timedelta(days=3)).strftime("%Y-%m-%d")
        ...
        if not jobs:
            ...
            if log and log.get("checkout_time"):
                ...
                elapsed = int((datetime.now() - ci_dt).total_seconds() / 60)
                ...
                with st.form("form_checkout"):
                    ...
                    if st.form_submit_button("🚪 CHECK-OUT — KẾT THÚC CA", use_container_width=True):
                        ...
                        if (datetime.now() - checkin_time).total_seconds() < 300:
                            ...
                            try:
                                ...
                                conn.execute("UPDATE logbook SET checkout_time=?,hoa_chat=?,ket_qua=?,attachments=? WHERE id=?",
                                             (datetime.now().isoformat(), hoa_chat, ket_qua, attachments_str, log["id"]))
                                conn.commit()
                                conn.close()
                ...
                time_preview = check_time_violation(job["gio_bat_dau"], job["gio_ket_thuc"], datetime.now().isoformat(), job["ngay_du_kien"])
                ...
                if not ktv or ktv == "(Chưa có)":
                    ...
                    if st.button("📍 CHECK-IN — BẮT ĐẦU CA THI CÔNG", use_container_width=True):
                        ...
                        if active_other:
                            ...
                            try:
                                tc = check_time_violation(job["gio_bat_dau"], job["gio_ket_thuc"], datetime.now().isoformat(), job["ngay_du_kien"])
                                conn.execute("""INSERT INTO logbook (schedule_id,ma_kh,ky_thuat_vien,checkin_time,canh_bao_gio)
                                                VALUES(?,?,?,?,?)""",
                                             (job["id"],job["ma_kh"],ktv,datetime.now().isoformat(), 1 if tc["violation"] else 0))
                                conn.execute("UPDATE schedules SET ky_thuat_vien=? WHERE id=?", (ktv, job["id"]))
                                conn.commit(); conn.close()
                                if tc["violation"]: st.warning(tc["message"])
                                st.success(f"✅ Check-in lúc {datetime.now().strftime('%H:%M')} — Chúc làm tốt!")
                                st.rerun()
        ...
        with tab_history:
            ...
            c1,c2 = st.columns(2)
            with c1: filter_date = st.date_input("Từ ngày", value=date.today()-timedelta(days=30))
