...
# app.py GÇö VHS CRM v4 GÇö Top navbar layout
...
<div class="vhs-nav">
  ...
  <div class="vhs-status">
    ...
    <div class="vhs-status-pill">GÜán+Å Nß+ú <b>{tong_no/1e6:.1f}M</b></div>
    <div class="vhs-status-pill">=ƒòÉ <b>{now.strftime('%H:%M')}</b></div>
  </div>
</div>
""", unsafe_allow_html=True)

# Radio nav GÇö nß¦¦m trong mß+Öt container -æß¦+c biß+çt -æ¦¦ß+úc CSS h+¦a th+ánh horizontal nav
NAV_ITEMS = [
    "=ƒÅá Dashboard",
    "=ƒæÑ Kh+ích H+áng",
    "=ƒôä Hß+úp -Éß+ông",
...
if page == "=ƒÅá Dashboard":
    ...
    <div style="background:linear-gradient(135deg,#0f172a 0%,#1e3a2f 60%,#166534 100%);
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
        <div>
          ...
          <div style="font-size:13px;color:#86efac;margin-top:3px;">{now.strftime('%A, %d/%m/%Y GÇö %H:%M')}</div>
        </div>
    ...
    with c4: st.metric("G£à Ca Ho+án Th+ánh",  ca_done)
    with c5: st.metric("GÜán+Å C+¦ng Nß+ú",        f"{tong_no/1e6:.1f}M -æ")
            ...
            for j in jobs_today:
                ...
                <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f8fafc;">
                  ...
                  <div style="flex:1;">
                    ...
                    <div style="font-size:11px;color:#94a3b8;">Lß¦ºn {j['lan_thu']} -+ {j['gio_bat_dau']}GÇô{j['gio_ket_thuc']}</div>
                  </div>
                </div>""", unsafe_allow_html=True)
        else:
            st.markdown('<div style="text-align:center;padding:24px;color:#94a3b8;font-size:13px;">G£¿ Kh+¦ng c+¦ ca n+áo h+¦m nay</div>', unsafe_allow_html=True)
            
        st.markdown("<hr style='margin:12px 0; border:0; border-top:1px solid #f1f5f9;'>", unsafe_allow_html=True)
        if st.button("GPín+Å -Éi tß+¢i Sß+ò Nhß¦¡t K++ (Logbook)", use_container_width=True):
    ...
    with col_b:
        ...
        """).fetchall()
        st.markdown('<div style="background:white;border:1px solid #e2e8f0;border-radius:14px;padding:18px;box-shadow:0 1px 4px rgba(0,0,0,.04);">', unsafe_allow_html=True)
        st.markdown('<div style="font-size:14px;font-weight:700;color:#0f172a;margin-bottom:12px;">GÜán+Å H-É Sß¦»p Hß¦+t Hß¦ín</div>', unsafe_allow_html=True)
        if expiring:
            for e in expiring:
                d  = e["days_left"]
            ...
            st.markdown('<div style="text-align:center;padding:24px;color:#94a3b8;font-size:13px;">G£à Tß¦Ñt cß¦ú H-É ß+òn</div>', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
